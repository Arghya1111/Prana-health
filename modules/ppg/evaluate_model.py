"""
STEP 7 — Evaluate the trained PPG CNN on the held-out test set.
...
from modules.ppg.utils import META_PATH, PPGDatasetInfo, ensure_model_dir, load_raw_dataset
...
def collect_predictions(
    ...
    """Return predicted labels, true labels, and softmax probabilities."""
    model.eval()
    all_preds: List[int] = []
    all_labels: List[int] = []
...
def plot_confusion_matrix(
    ...
    ax.set_ylabel("True")
    ax.set_title("PPG CNN — Confusion Matrix")
...
def plot_roc_curve(
    ...
    """Save ROC curve plot (binary or one-vs-rest)."""
    ...
    if n_classes == 2:
        ...
        roc_auc = auc(fpr, tpr)
        ax.plot(fpr, tpr, color="#C0392B", lw=2, label=f"ABNORMAL (AUC = {roc_auc:.3f})")
    else:
    ...
    ax.set_title("PPG CNN — ROC Curve")
...
def print_metrics(
    ...
    print("\n" + "=" * 60)
    print("  PRANA PPG - Model Evaluation")
    print("=" * 60)
...
def main() -> None:
    ...
    try:
        model, meta = load_ppg_model(device=device)

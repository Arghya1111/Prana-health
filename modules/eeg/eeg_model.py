"""
STEP 4 — 1D CNN architecture for EEG classification.

Modular design: stack depth, channel widths, and dropout are configurable.
"""
...
class EEG1DCNN(nn.Module):
    """
    1D CNN for EEG window classification.
    ...
    num_classes : int
        Number of output classes (2 for binary NORMAL/ABNORMAL).
    conv_channels : list of int
        Output channels for each ConvBlock.
    kernel_size : int
        Convolution kernel width.
    pool_size : int
        MaxPool reduction factor per block.
    fc_hidden : int
        Hidden units in the fully-connected layer.
    dropout : float
        Dropout probability before final linear layer.
    """

...
from app.paths import DB_PATH, MODEL_PATH, LABELED_DATASET_CSV
...
def save_fusion_report(fusion_result, db_path=DB_PATH):
    """
    Persist a Phase 8 fusion result to the fusion_reports table.

    Parameters
    ----------
    fusion_result:
        ``FusionResult`` from ``fusion.fusion_engine`` or a compatible dict.
    """
    if hasattr(fusion_result, "to_dict"):
        data = fusion_result.to_dict()
        profile = fusion_result.profile
        risk = fusion_result.risk.risk_score
        ...
        rec_text = fusion_result.recommendation.combined_recommendation
        module_count = profile.module_count()
        modules = {k: v.to_dict() if hasattr(v, "to_dict") else v
                   for k, v in profile.modules.items()}
    else:
        data = fusion_result
        profile_data = data.get("report", data)
        risk = data.get("risk_score", 0)
        severity = data.get("overall_severity", "NORMAL")
        confidence = data.get("overall_confidence", 0)
        rec = data.get("recommendation", {})
        action = rec.get("action", "")
        rec_text = rec.get("combined_recommendation", "")
        module_count = len(profile_data.get("modules", {}))
        modules = profile_data.get("modules", {})

    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    patient_id = (
        fusion_result.profile.patient_id
        if hasattr(fusion_result, "profile")
        else data.get("patient_id", "UNKNOWN")
    )
    ...
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO fusion_reports (
            patient_id, timestamp,
            risk_score, overall_severity, overall_confidence,
            recommendation, action, module_count,
            ecg_status, eeg_status, ppg_status, spo2_status,
    ...
    """, (
        ...
        rec_text, action, module_count,
        _mod_status("ECG"), _mod_status("EEG"), _mod_status("PPG"),
        _mod_status("SpO2") or _mod_status("SPO2"),
        _mod_status("THERMAL"), _mod_status("OPTICAL"), _mod_status("XRAY"),
        json.dumps(data, default=str),
    ))
    conn.commit()
    row_id = cursor.lastrowid
    conn.close()
    print(f"  Fusion report saved -> DB row #{row_id}  "
          f"(Patient: {patient_id}, Severity: {severity}, Risk: {risk})")
    return row_id


def get_fusion_reports(db_path=DB_PATH, limit=50):
    """Return recent fusion reports, newest first."""
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("""
        SELECT * FROM fusion_reports
...
def get_fusion_report(report_id, db_path=DB_PATH):
    """Return a single fusion report by ID."""
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM fusion_reports WHERE id = ?", (report_id,))
    row = cursor.fetchone()
...
def get_patient_fusion_history(patient_id, db_path=DB_PATH):
    """Return all fusion reports for a patient, newest first."""
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("""
        SELECT * FROM fusion_reports
...
def get_fusion_stats(db_path=DB_PATH):
    """Count fusion reports by overall severity."""
    conn = sqlite3.connect(db_path)
    cursor = conn.cursor()
    cursor.execute("""
